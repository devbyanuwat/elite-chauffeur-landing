...
> ตัวเลข **4.59:1** / **4.73:1** ที่เคยลงไว้ตรงนี้ถูกสุ่มกลางคัน
> `transition: opacity .4s` (ที่ opacity ~0.73 ของตัวอักษร ไม่ใช่ค่าตอนนิ่ง)
> จึงต่ำเกินจริง แก้ไขใหม่ (2026-08-04, residual fix) — คำนวณจาก token จริง
> ใน `Why.astro`/`global.css` ตอนนิ่ง (ไม่มี transition ค้าง) แล้วยืนยันด้วย
> screenshot จริงผ่าน `chromium_headless_shell-1208` (อ่านค่าพิกเซลด้วย
> ImageMagick ไม่ใช่แค่คำนวณเฉย ๆ):
>
> - **การ์ดที่ผ่านมาแล้ว (`.entered`, ไม่ใช่ `.current`):** พื้นหลังคือ
>   `--bg-dark-soft` (`oklch(28% .020 230)` = `#1f2b31`, ทึบ — screenshot
>   ยืนยันตรงกับค่าคำนวณเป๊ะ) ตัวอักษรเนื้อความ `oklch(76% .012 230)` =
>   `#aab3b7` ให้คอนทราสต์ **6.80:1** (ผ่าน WCAG 1.4.3 ที่ 4.5:1 สบาย ๆ;
>   ของเดิมก่อน Fix 12 ประมาณ 2.2:1)
> - **การ์ดปัจจุบัน (`.current`):** `background: oklch(64% .115 48 / .14)`
>   ใน `.why-chapter.pin-ready .why-item.current` (specificity 4 คลาส) เข้ามา
>   *แทนที่* `background: var(--bg-dark-soft)` ของ `.why-item` (specificity
>   1 คลาส) ทั้งค่า ไม่ใช่วางซ้อนทับ — สีที่ผสมจริงจึงคอมโพสิตกับพื้นหลังของ
>   section เอง (`--bg-dark` = `oklch(20% .022 235)` = `#0c181f`) ไม่ใช่กับ
>   `--bg-dark-soft` ของการ์ด ได้พิกเซลจริง `#262525` (screenshot ยืนยันตรงกับ
>   ค่าคำนวณ ต่างจาก `#363535` ที่จะได้ถ้าสมมติผิดว่าวางซ้อนบน
>   `--bg-dark-soft`) → คอนทราสต์ **7.16:1** พร้อมกรอบทอง
>   `oklch(0.78 0.085 55)` + box-shadow — ค่าเท่ากันทั้ง 390×844 และ 1440×900
>
